...
def subsample_reference(refname, reflength, bamfile, depth, allow_orphans=False):
    ...
    ''' 
    regionstr = '{0}:{1}-{1}'.format(refname, 1, reflength)
    orphans = '-A' if allow_orphans else '' 
    bamfile = '"{0}"'.format(bamfile)
    subsam_cmd = "python subsample_mindepth.py {0} {1} --subsample {2} {3}".format(bamfile, refname, reflength,  depth, orphans)
    filename = "{0}.min.{1}".format(refname, str(depth))
    index_cmd  = "samtools view -hSb - | samtools sort - {0}.bam; samtools index {0}.bam;".format(filename)
    ...
    with open('stderr.log', 'w') as errlog:
        ...
        index_process = sp.Popen(
                ...
                stdout=sp.PIPE,
                stderr=errlog,
                stdin=subsample_proc.stdout, 
                shell=True) 
        output = index_process.communicate()[0]
        print output
    return
